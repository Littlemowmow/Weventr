...
import {
  ArrowLeft, MapPin, Calendar, DollarSign, Clock, Heart, X, Check,
  Sparkles, Star, Users, Send, ChevronRight, Grip, AlertCircle,
  Flame, ThumbsUp, HelpCircle, Zap, Home as HomeIcon, Utensils, Plane, Ticket
} from "lucide-react";
    ...
    <div className="relative mx-auto select-none shrink-0" style={{ width: PHONE_W, height: PHONE_H }}>
      <div
        ...
        style={{
          ...
          borderRadius: SCREEN_RADIUS, background: "#0c0c0e", zIndex: 1,
        }}
      >
        <div className="flex items-center justify-between px-5 pt-3 pb-1">
          <span className="text-white text-[11px] font-semibold">9:41</span>
          <div className="flex items-center gap-1">
            <svg width="14" height="10" viewBox="0 0 16 11" fill="white"><rect x="0" y="4" width="3" height="7" rx=".8"/><rect x="4.5" y="2.5" width="3" height="8.5" rx=".8"/><rect x="9" y="1" width="3" height="10" rx=".8"/><rect x="13.5" y="0" width="2.5" height="11" rx=".8" opacity=".3"/></svg>
            <svg width="13" height="10" viewBox="0 0 15 11" fill="white"><circle cx="7.5" cy="9.5" r="1.5"/><path d="M4 6.5A5 5 0 0111 6.5" stroke="white" strokeWidth="1.5" fill="none" strokeLinecap="round"/><path d="M1.5 4A8.5 8.5 0 0113.5 4" stroke="white" strokeWidth="1.5" fill="none" strokeLinecap="round"/></svg>
            <svg width="22" height="11" viewBox="0 0 25 12" fill="none"><rect x=".5" y=".5" width="21" height="11" rx="3.5" stroke="white" strokeOpacity=".35"/><rect x="1.5" y="1.5" width="18" height="9" rx="2.5" fill="white"/><path d="M23 4v4a2 2 0 000-4z" fill="white" opacity=".4"/></svg>
          </div>
        </div>
        <div className="flex justify-center mb-1">
          <div style={{ width: 100, height: 26, borderRadius: 18, background: "#000", boxShadow: "0 0 0 1px rgba(255,255,255,0.06)" }} />
        </div>
        <div className="px-3 pb-3 overflow-hidden" style={{ height: SCREEN_H - 76 }}>
          {children}
        </div>
      </div>
      <svg width={PHONE_W} height={PHONE_H} viewBox={`0 0 ${PHONE_W} ${PHONE_H}`} style={{ position: "absolute", top: 0, left: 0, zIndex: 10, pointerEvents: "none" }}>
        <defs>
          <linearGradient id="fgA" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#8e8e93"/><stop offset="20%" stopColor="#aeaeb2"/><stop offset="45%" stopColor="#6c6c70"/><stop offset="70%" stopColor="#9a9a9f"/><stop offset="100%" stopColor="#3a3a3c"/>
          </linearGradient>
          <linearGradient id="fgT" x1="0%" y1="0%" x2="0%" y2="100%">
            <stop offset="0%" stopColor="rgba(255,255,255,0.18)"/><stop offset="100%" stopColor="rgba(255,255,255,0)"/>
          </linearGradient>
          <mask id="fmask">
        ...
        <rect width={PHONE_W} height={PHONE_H} rx={FRAME_RADIUS} fill="url(#fgA)" mask="url(#fmask)"/>
        <rect width={PHONE_W} height={PHONE_H} rx={FRAME_RADIUS} fill="url(#fgT)" mask="url(#fmask)"/>
        <rect width={PHONE_W} height={PHONE_H} rx={FRAME_RADIUS} fill="none" stroke="rgba(255,255,255,0.16)" strokeWidth="1"/>
        <rect x={SCREEN_INSET} y={SCREEN_INSET} width={SCREEN_W} height={SCREEN_H} rx={SCREEN_RADIUS} fill="none" stroke="rgba(255,255,255,0.07)" strokeWidth="1"/>
      ...
      <div className="absolute inset-0 rounded-[50px] pointer-events-none" style={{ zIndex: 0, boxShadow: "0 40px 100px rgba(0,0,0,0.8)" }}/>
    </div>
  );
}
...
function GlassRow({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`px-2.5 py-2 rounded-xl ${className}`} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
      {children}
    </div>
  );
}

function OrangeButton({ children, small = false }: { children: React.ReactNode; small?: boolean }) {
  return (
    <div
      className={`text-center font-bold text-white rounded-xl ${small ? "text-[11px] py-2 px-3" : "text-[13px] py-3 px-4"}`}
...
function GhostButton({ children }: { children: React.ReactNode }) {
  return (
    <div className="text-center text-[12px] font-semibold text-white/40 py-2.5 px-3 rounded-xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.08)" }}>
        <div className="h-full rounded-full" style={{ width: `${pct}%`, background: "linear-gradient(to right, #f97316, #f59e0b)" }} />
      </div>
    </div>
  );
}
...
function TripSparkCreate() {
  return (
    <PhoneFrame>
      ...
      <OrangeButton>Create Trip Spark ⚡</OrangeButton>
    ...
    <PhoneFrame>
      ...
      <div className="mt-3 mb-2">
        ...
        <div className="grid grid-cols-7 gap-[3px] text-center mb-1">
          {days.map(d => <div key={d} className="text-[8px] text-white/25 font-bold">{d}</div>)}
        </div>
      ...
      <div className="flex gap-2">
        <div className="flex-1"><OrangeButton small>I'm in ✓</OrangeButton></div>
        <div className="flex-1"><GhostButton>Maybe 🤷</GhostButton></div>
      </div>
    </PhoneFrame>
  );
}

function TripSparkStatus() {
  const people = [
    { name: "You", status: "in", time: "2h ago", emoji: "😎" },
    { name: "Sarah", status: "in", time: "1h ago", emoji: "👩" },
    { name: "Mike", status: "maybe", time: "45m ago", emoji: "🧑" },
    { name: "Jess", status: "waiting", time: "", emoji: "👧" },
    { name: "Marcus", status: "holding", time: "", emoji: "🧔" },
  ];
  return (
    <PhoneFrame>
        ...
        {people.map((p, i) => (
          <GlassRow key={i}>
            <div className="flex items-center gap-2">
              ...
              {p.status === "maybe" && (
                <span className="text-amber-400 text-[10px] font-semibold">Maybe</span>
              )}
              {p.status === "waiting" && (
                <span className="text-white/30 text-[10px]">⏳ Waiting</span>
              )}
              {p.status === "holding" && (
                <span className="text-red-400 text-[10px] font-bold">🚨 Holding up the group</span>
              )}
            </div>
          </GlassRow>
        ))}
      ...
      <div className="mt-3">
        <GhostButton>View Results →</GhostButton>
      </div>
    </PhoneFrame>
  );
}

function TripSparkResults() {
  return (
    <PhoneFrame>
      <div className="text-center mb-3">
      ...
      <div className="rounded-xl p-3 mb-2" style={{ background: "rgba(52,211,153,0.08)", border: "1px solid rgba(52,211,153,0.2)" }}>
      ...
      <div className="rounded-xl p-3 mb-2" style={{ background: "rgba(249,115,22,0.08)", border: "1px solid rgba(249,115,22,0.2)" }}>
        ...
        <div className="space-y-1.5">
          {[
            ...
            <div key={i} className="flex items-center justify-between">
              <div className="flex items-center gap-1.5">
                {b.icon}
                <span className="text-white/50 text-[10px]">{b.label}</span>
              </div>
      ...
      <div className="mt-2">
        ...
        <OrangeButton>Let's plan this trip! →</OrangeButton>
      </div>
    </PhoneFrame>
  );
}
...
function BlindMatchSwipe() {
  return (
    <PhoneFrame>
      ...
      <div className="relative rounded-2xl overflow-hidden mb-2.5" style={{ height: 220, border: "1px solid rgba(255,255,255,0.08)" }}>
        ...
        <div className="absolute top-2.5 left-2.5">
          <div className="px-2 py-0.5 rounded-full text-[9px] font-bold" style={{ background: "rgba(249,115,22,0.2)", color: "#f97316", border: "1px solid rgba(249,115,22,0.3)" }}>
            🗺 SideQuest
          </div>
        </div>
        ...
        <div className="absolute bottom-2.5 left-2.5 right-2.5">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            <div className="flex items-center gap-1 text-[10px] text-white/55">
              <MapPin size={8}/><span>El Born</span>
            </div>
      ...
      <div className="flex items-center justify-center gap-1 mb-1">
        <div className="text-[9px] text-white/20">← NOPE</div>
        <div className="w-16"/>
        <div className="text-[9px] text-white/20">LIKE →</div>
      </div>
      <div className="text-center text-[9px] text-white/15">↑ SUPER LIKE</div>
    </PhoneFrame>
  );
}

function BlindMatchReveal() {
  return (
    <PhoneFrame>
      <div className="text-center mb-3">
      ...
      <div className="mb-3">
        ...
        <div className="rounded-xl p-2.5 mb-1.5" style={{ background: "rgba(249,115,22,0.08)", border: "1px solid rgba(249,115,22,0.2)" }}>
          <div className="flex items-center gap-2.5">
            ...
            <div className="shrink-0"><OrangeButton small>Book →</OrangeButton></div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-3">
        ...
        <div className="rounded-xl p-2.5" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.05)" }}>
          <div className="flex items-center gap-2.5">
            ...
            <div className="px-2 py-1 rounded-full text-[9px] font-bold" style={{ background: "rgba(139,92,246,0.15)", color: "#a78bfa", border: "1px solid rgba(139,92,246,0.25)" }}>
              AI Tiebreaker?
            </div>
          </div>
        </div>
      ...
      <OrangeButton small>Build itinerary with matches →</OrangeButton>
    </PhoneFrame>
  );
}


function BudgetLockScreen() {
  return (
    <PhoneFrame>
      <div className="text-white font-bold text-[14px] mb-1">Set Your Budget</div>
      <div className="text-white/35 text-[11px] mb-3">Barcelona · 5 nights · Jul 14-18</div>
      ...
      <OrangeButton>Lock My Budget 🔒</OrangeButton>
    </PhoneFrame>
  );
}

function BudgetSweetSpot() {
  const people = [
...
function SquadItineraryScreen() {
  const items = [
    { time: "9 AM", activity: "La Boqueria Market", icon: "🍊", by: "Jess", color: "rgba(249,115,22,0.15)", budget: "Free", status: "within" },
    { time: "1 PM", activity: "Gothic Quarter Walk", icon: "🏛️", by: "Marco", color: "rgba(139,92,246,0.15)", budget: "Free", status: "within" },
    { time: "4 PM", activity: "El Born Food Tour", icon: "🍷", by: "Sarah", color: "rgba(14,165,233,0.15)", budget: "$45", status: "exceeds" },
    { time: "8 PM", activity: "Sunset Viewpoint", icon: "🌅", by: "You", color: "rgba(52,211,153,0.15)", budget: "Free", status: "within" },
  ];
  return (
    <PhoneFrame>
      <div className="flex items-center justify-between mb-2">
      ...
      <div className="space-y-1.5">
        {items.map((item, i) => (
          <div key={i} className="flex items-center gap-2.5 p-2.5 rounded-xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
            <div className="w-8 h-8 rounded-lg flex items-center justify-center text-sm shrink-0" style={{ background: item.color }}>
              {item.icon}
            </div>
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                <span className={`text-[9px] font-semibold ${item.status === "within" ? "text-emerald-400" : "text-amber-400"}`}>{item.budget}</span>
              </div>
            </div>
  ...
  return (
    <PhoneFrame>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="px-2 py-1 rounded-full text-[9px] font-bold" style={{ background: "rgba(249,115,22,0.15)", color: "#f97316" }}>
          + Add Task
        </div>
      </div>
...
const PHASES = [
  ...
  },
  {
    ...
    screens: [
      { component: <BudgetLockScreen />, step: 1, label: "Set Your Caps" },
      { component: <BudgetSweetSpot />, step: 2, label: "Group Sweet Spot" },
    ],
  },
  {
    id: "squad-itinerary",
    ...
    screens: [
      { component: <SquadItineraryScreen />, step: 1, label: "Day-by-Day Plan" },
      { component: <TaskDelegationScreen />, step: 2, label: "Task Delegation" },
    ],
  },
];
